  ...
  <div class="backdrop" @click.self="closeModal">
    <div class="modal">
      <form @submit.prevent="handleSubmit">
        ...
        <textarea
          rows="3"
          v-model="materialToEdit.description"
...
export default {
  props: ["material"],
  setup(props, context) {
    const { editById, getSubItems, deleteById } = useCRUD();
    const { uploadFile } = useUpDownFile();

    const materialToEdit = reactive({
      name: props.material.name,
      description: props.material.description,
      courseId: props.material.courseId,
    });
    ...
    const getFilesForMaterial = async () => {
      files.value = await getSubItems("files", "material", props.material.id);
    };
    ...
    const handleSubmit = async () => {
      uploadNewFiles(props.material.id);
      deleteFilesFromBe();
      ...
      props.material.name = materialToEdit.name;
      props.material.description = materialToEdit.description;
